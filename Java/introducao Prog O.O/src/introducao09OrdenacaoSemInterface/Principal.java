			...
			{	case 1:
				...
				case 6:	// Lista empregados ordenados ascendentemente por salário. 
					...
					if (tam > 0)
					{
						...
						Empregado[] arrayEmpregados = new Empregado[tam];
						
						arrayEmpregados[0] = listaDeEmpregados.recuperarPrimeiro();
						
						for(int i=1;i<tam; i++){
							arrayEmpregados[i] = listaDeEmpregados.recuperarProximo();
						}
						Ordenacao.ordena(arrayEmpregados);
						
						for(int i=0;i<tam;i++){
							arrayEmpregados[i].imprime();
						}
					}
					else
						System.out.println('\n' + "Não há empregados na lista."); 
